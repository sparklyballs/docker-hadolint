...
environment {
	...
	CONTAINER_REPOSITORY = 'sparklyballs/hadolint'
	GITHUB_RELEASE_URL_SUFFIX = 'hadolint/hadolint/commits/master'
	GITHUB_REPOSITORY = 'sparklyballs/docker-hadolint'
...
script{
	...
	env.RELEASE_VER = "$RELEASE_VER"[0..7]
        env.GPM_RELEASE_VER = sh(script: 'curl -s "https://gmplib.org/#DOWNLOAD" | grep lz | grep -oE '[0-9]+([.][0-9]+)+([.][0-9]+)?' | head -n 1', returnStdout: true).trim()
	}
	}
	}
...
steps {
	...
	recordIssues enabledForFailure: true, tool: checkStyle(pattern: 'hadolint-result.xml')	
	}
	}
